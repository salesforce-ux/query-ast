...
module.exports = (ast, options) => {
   ...
   */
  let $ = (...args) => new QueryWrapper(...args)

  /**
   * Wrap a {@link NodeWrapper} or {@link NodeWrapper} array with chainable traversal/mutation functions
   */
  class QueryWrapper {
    /**
     ...
     *
     * @private
     * @param {NodeWrapper|NodeWrapper[]} node
     * @param {QueryWrapper} [prevWrapper]
     */
    constructor (node, prevWrapper) {
      if (!node) node = ROOT
      let nodes = _.flatten([node])
      invariant(_.every(nodes, NodeWrapper.isNodeWrapper),
        'Only NodeWrapper instances can be passed in to a Wrapper')
      invariant(!prevWrapper || prevWrapper instanceof QueryWrapper,
        'prevWrapper must be an instance of QueryWrapper')
      this.nodes = nodes
